...
int main(int argc,char* argv[]){
    ...
    dim3 grids_mix(2,1,unitdimX*unitdimY);
        ...
        monitor2_lastdata_store<<<grids_mix,blocks_mix>>>(eta2,eta2_last);
    #endif
    cudaDeviceSynchronize();
    ...
    for(int i=0;i<5;i++){
        ...
        cudaDeviceSynchronize();
        kernel1_conversion<<<grids_mix,blocks_mix>>>(eta1_out,heta1_out,type_curr,i);
        cudaDeviceSynchronize();
        ...
        cudaDeviceSynchronize();
        kernel1_conversion<<<grids_mix,blocks_mix>>>(eta2_out,heta2_out,type_curr,i);
        cudaDeviceSynchronize();
        swap(eta1,eta1_out);
        swap(eta2,eta2_out);
