...
for datatype in ["double","float"]:
    for para in range(1,6):
        os.system("rm -rf ./time/"+datatype+"/para"+str(para)+"/with_M1_monitor")
        os.system("rm -rf ./time/"+datatype+"/para"+str(para)+"/with_M2_monitor")
for datatype in ["double","float"]:
    thresholds=None
    for monitor in range(1,3):
        ...
        for threshold in thresholds:
            ...
            file.close()
            for para in range(1,6): 
                timepath="./time/"+datatype+"/para"+str(para)+"/with_M"+str(monitor)+"_monitor/"
                ...
                file.writelines(flist)
                file.close()     
                if os.system("nvcc monitor_independent_withmonitor.cu -o monitor_independent_withmonitor --std=c++11 -arch=sm_80")==0:
                    listtime=[str(threshold)]
                    for i in range(5):
                        os.system("./monitor_independent_withmonitor")
